/* Overview — the workspace at a glance. */

import { h, barChart, ago, fmtDate, num, meter } from '../../lib/ui.js';
import {
  quarterRevenue, pipelineValue, openDeals, overdueInvoices, outstandingValue,
  revenueByMonth, customerName, seatsUsed, changesSince, atRiskCustomers, stageLabel,
  activityText, dealTitle, industryLabel,
} from '../data.js';
import { t } from '../main.js';
import { pageHead, statCard, wsMoney, statusPill, sectionTitle, iconEl, plural } from '../parts.js';

export function render(ctx) {
  const { ws, navigate } = ctx;
  const q = quarterRevenue(ws);
  const overdue = overdueInvoices(ws);
  const active = ws.customers.filter((c) => c.status === 'active');
  const risk = atRiskCustomers(ws);
  const week = changesSince(ws, 7);
  const money = (n) => wsMoney(ws, n);

  const soon = openDeals(ws)
    .filter((d) => new Date(d.closeDate).getTime() - Date.now() < 21 * 86400000)
    .sort((a, b) => new Date(a.closeDate) - new Date(b.closeDate))
    .slice(0, 5);

  const root = h('div', { class: 'view view--pad' });

  root.appendChild(pageHead(
    t('overview.title', { name: ws.name }),
    t('overview.sub', { industry: industryLabel(ws.industry), city: ws.city }),
    [
      h('button', { class: 'btn', onclick: () => navigate('reports') }, iconEl('chart'), t('overview.reports')),
      h('button', { class: 'btn btn--primary', onclick: () => navigate('deals') }, iconEl('bolt'), t('overview.pipeline')),
    ],
  ));

  root.appendChild(h('div', { class: 'grid g4' },
    statCard(t('overview.revenue', { q: q.label }), money(q.total), t('overview.settled', { n: q.count }), true),
    statCard(t('overview.pipeline'), money(pipelineValue(ws)), t('overview.inPlay', { n: openDeals(ws).length })),
    statCard(t('overview.overdue'), money(overdue.reduce((sum, i) => sum + i.amount, 0)), t('overview.pastDue', { n: overdue.length })),
    statCard(t('overview.activeCustomers'), num(active.length), t('overview.onFile', { n: ws.customers.length }))));

  /* revenue chart + seat usage */
  const chartCard = h('div', { class: 'card' },
    h('div', { class: 'card__head' },
      h('h3', {}, t('overview.chart')),
      h('span', { class: 'label' }, ws.currency)),
    barChart(revenueByMonth(ws, 6), { format: (v) => money(v) }));

  const used = seatsUsed(ws);
  const seatCard = h('div', { class: 'card stack' },
    h('div', { class: 'card__head' }, h('h3', {}, t('overview.health'))),
    h('div', {},
      h('div', { class: 'between small' }, h('span', {}, t('overview.seatsUsed')), h('span', { class: 'num' }, `${used} / ${ws.seatsIncluded}`)),
      h('div', { style: 'margin-top:6px' }, meter(used, ws.seatsIncluded, used > ws.seatsIncluded ? 'bad' : ''))),
    h('div', {},
      h('div', { class: 'between small' }, h('span', {}, t('overview.atRisk')), h('span', { class: 'num' }, `${risk.length} / ${ws.customers.length}`)),
      h('div', { style: 'margin-top:6px' }, meter(risk.length, ws.customers.length, 'bad'))),
    h('div', {},
      h('div', { class: 'between small' }, h('span', {}, t('overview.outstanding')), h('span', { class: 'num' }, money(outstandingValue(ws)))),
      h('div', { style: 'margin-top:6px' }, meter(overdue.length, ws.invoices.length, 'bad'))),
    h('p', { class: 'small muted' }, t('overview.week', { n: week.length })));

  root.appendChild(h('div', { class: 'grid g-side', style: 'margin-top:20px' }, chartCard, seatCard));

  /* open items */
  root.appendChild(sectionTitle(t('overview.openItems'),
    h('button', { class: 'btn btn--sm', onclick: () => navigate('invoices') }, t('overview.allInvoices'))));

  const items = h('div', { class: 'grid g2' });

  const overdueList = h('div', { class: 'card' },
    h('div', { class: 'card__head' }, h('h3', {}, t('overview.overdueList')), statusPill('overdue', `${overdue.length}`)));
  if (!overdue.length) overdueList.appendChild(h('p', { class: 'muted small' }, t('overview.overdueEmpty')));
  else {
    overdueList.appendChild(h('ul', { class: 'itemlist' }, overdue.slice(0, 6).map((i) => h('li', {},
      h('div', { style: 'flex:1;min-width:0' },
        h('div', { class: 'truncate' }, customerName(ws, i.customerId)),
        h('div', { class: 'faint small mono' }, t('overview.dueOn', { number: i.number, date: fmtDate(i.dueAt) }))),
      h('span', { class: 'num' }, money(i.amount))))));
  }

  const dealList = h('div', { class: 'card' },
    h('div', { class: 'card__head' }, h('h3', {}, t('overview.closing')), h('span', { class: 'pill' }, `${soon.length}`)));
  if (!soon.length) dealList.appendChild(h('p', { class: 'muted small' }, t('overview.closingEmpty')));
  else {
    dealList.appendChild(h('ul', { class: 'itemlist' }, soon.map((d) => h('li', {},
      h('div', { style: 'flex:1;min-width:0' },
        h('div', { class: 'truncate' }, dealTitle(ws, d)),
        h('div', { class: 'faint small mono' }, `${stageLabel(d.stage)} · ${d.owner} · ${fmtDate(d.closeDate)}`)),
      h('span', { class: 'num' }, money(d.value))))));
  }

  items.append(overdueList, dealList);
  root.appendChild(items);

  /* activity */
  root.appendChild(sectionTitle(t('overview.activity')));
  const feed = h('div', { class: 'card' },
    h('ol', { class: 'timeline' }, ws.activity.slice(0, 12).map((a) => h('li', { class: 'timeline__item' },
      h('div', { class: 'between' },
        h('strong', { style: 'font-weight:600' }, activityText(a)),
        h('span', { class: 'faint small mono' }, ago(a.at))),
      h('div', { class: 'faint small' }, `${a.actor} · ${t(`activityType.${a.type}`)}`)))));
  root.appendChild(feed);

  return root;
}

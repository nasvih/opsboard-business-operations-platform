...
/* The four sentences this module renders. An app hands over its own
   translations through `labels`; the English below stands if it does not. */
...
export function initPWA({ mount, appName = 'this app', swPath = './sw.js', onNote, labels } = {}) {
  ...
  const say = (k) => String(L[k]).replace('{app}', appName);
  ...
  btn.innerHTML = `${ICON}<span>${say('install')}</span>`;
  btn.hidden = !isIOS();               // shown at once on iOS, on prompt elsewhere
  btn.title = say('installTitle');
  btn.setAttribute('aria-label', say('installTitle'));
  ...
  window.addEventListener('appinstalled', () => {
    deferred = null;
    btn.hidden = true;
    if (onNote) onNote(say('installed'));
  });
  ...
  btn.addEventListener('click', async () => {
    if (deferred) {
      ...
      if (outcome !== 'accepted' && onNote) onNote(say('dismissed'));
      deferred = null;
      return;
    }
    if (onNote) {
      onNote(isIOS() ? say('ios') : say('other'));
    }
  });

...
export class Assistant {
  constructor(cfg) {
    this.cfg = Object.assign({
      ...
      context: () => ({}),
      /* The chrome the engine renders itself. An app hands over its own
         translations; the English below is what stands if it does not. */
      labels: {},
    }, cfg);
    this.cfg.labels = Object.assign({
      open: 'Open {name}',
    ...
    this.L = (k, vars) => String(this.cfg.labels[k]).replace(/\{(\w+)\}/g, (_, v) => (vars && vars[v] !== undefined ? vars[v] : ''));
    this.log = [];
    this.open = false;
    this.busy = false;
  }
  ...
  mount(root = document.body) {
    ...
    fab.type = 'button';
    fab.title = this.L('fabTitle', { name: this.cfg.name });
    fab.setAttribute('aria-label', this.L('open', { name: this.cfg.name }));
    ...
    host.innerHTML = `
      <section class="assist${docked ? ' assist--docked' : ''}" role="dialog" aria-label="${esc(this.cfg.name)}">
        <header class="assist__head">
          ...
          <button class="btn btn--ghost btn--icon" data-act="reset" title="${esc(this.L('clear'))}" aria-label="${esc(this.L('clear'))}">${ICON.reset}</button>
          ${docked ? '' : `<button class="btn btn--ghost btn--icon" data-act="close" title="${esc(this.L('close'))}" aria-label="${esc(this.L('close'))}">${ICON.close}</button>`}
        </header>
        <div class="assist__log" data-log></div>
        <div class="assist__suggest" data-suggest></div>
        <form class="assist__form" data-form>
          <input class="input" placeholder="${esc(this.L('placeholder'))}" aria-label="${esc(this.L('placeholder'))}" autocomplete="off" data-input>
          <button class="btn btn--primary btn--icon" type="submit" title="${esc(this.L('send'))}" aria-label="${esc(this.L('send'))}">${ICON.send}</button>
        </form>
  ...
  _push(who, html, meta) {
    ...
    el.innerHTML = `<span class="msg__av">${who === 'me' ? esc(this.L('you')) : esc(this.cfg.initials)}</span>
  ...
  async ask(q) {
    ...
    catch (err) { out = { text: this.L('edge') }; }
  ...
  _renderActions(body, actions) {
    ...
    actions.forEach((a, i) => {
      ...
      btn.addEventListener('click', async () => {
        ...
        btn.textContent = a.doingLabel || this.L('working');
        let result;
        try { result = await a.run(); } catch (err) { result = { text: this.L('failed') }; }
        if (typeof result === 'string') result = { text: result };
        row.remove();
        const done = this._push('bot', '', '');
        done.innerHTML = render(result.text || this.L('done'));
        if (result.table) done.innerHTML += tableHTML(result.table);
        done.innerHTML += `<div class="msg__meta">${esc(result.meta || this.L('applied'))}</div>`;
  ...
  _route(q) {
    ...
    if (best) return best;
    return { answer: () => ({ text: pick(this.cfg.fallbacks), suggestions: this.cfg.suggestions }), trace: this.L('searched') };
  }
}

...
/* Dates follow the language written onto <html> before the first paint.
   `ar-u-nu-latn` gives Arabic month names with Latin digits, so a date still
   lines up in a column beside amounts, ids and invoice numbers. Money is
   deliberately left alone above: the symbol and the digits do not translate. */
export const dateLocale = () => (document.documentElement.lang === 'ar' ? 'ar-u-nu-latn' : 'en-GB');

/* The relative words come from the app's dictionary — main.js hands them over
   at boot. The English defaults stand until it does, and stand for good in any
   app that never calls it. */
let AGO = { now: 'just now', m: '{n}m ago', h: '{n}h ago', d: '{n}d ago' };
/* Labels the kit itself renders, for the same reason. */
let UI = { cancel: 'Cancel', confirm: 'Confirm', close: 'Close' };
export const setAgoStrings = (o) => { AGO = { ...AGO, ...(o || {}) }; };
export const setUiStrings = (o) => { UI = { ...UI, ...(o || {}) }; };

export function fmtDate(d, opts) {
  const dt = d instanceof Date ? d : new Date(d);
  return dt.toLocaleDateString(dateLocale(), opts || { day: '2-digit', month: 'short', year: 'numeric' });
}
...
export function modal({ title, body, actions = [], width }) {
  ...
  box.appendChild(h('div', { class: 'modal__head' },
    h('h3', {}, title || ''),
    h('button', {
      class: 'btn btn--ghost btn--icon', onclick: close,
      'aria-label': UI.close, title: UI.close,
...
export function confirmDialog(message, { title = UI.confirm, danger = false, okLabel = UI.confirm } = {}) {
  return new Promise((resolve) => {
    modal({
      title,
      body: `<p class="muted">${esc(message)}</p>`,
      actions: [
        { label: UI.cancel, class: '', onClick: () => resolve(false) },
